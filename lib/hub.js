...
class Hub extends EventEmitter {
    constructor(parent, oaObject) {
        ...
        this.websocket = function () {
            hubConnection = new WebSocket('ws://' + this.ip + ':8088/?domain=svcs.myharmony.com&hubId=' + this.remoteId);
    ...
    keepAlive() {
        var timeout = 10000;
        if (hubConnection.readyState == WebSocket.OPEN) {
    ...
    sendRequest(command, params, mssgId) {
        ...
        hubConnection.send(JSON.stringify(payload), (err) => {
            if (err) {
                console.log(err);
            }
        });
